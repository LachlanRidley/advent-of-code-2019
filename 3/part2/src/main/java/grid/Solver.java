...
public class Solver {
    ...
    public int solvinate(String puzzle) {
        String line1 = puzzle.split("\n")[0];
        String line2 = puzzle.split("\n")[1];

        List<Coords> line1Coords = findLocations(line1);
        List<Coords> line2Coords = findLocations(line2);

        return findSmallestWeight(line1Coords, line2Coords);
    }

    public List<Coords> findLocations(String instructionSet) {
        String[] steps = instructionSet.split(",");

        List<Instruction> instructions = Arrays.stream(steps).map(Instruction::from).collect(Collectors.toList());

        List<Coords> visited = new ArrayList<>();
        Coords current = ORIGIN;
        visited.add(current);
            ...
            switch (instruction.dir) {
                case 'U':
                    for (int i = 0; i < instruction.length; i++) {
                        stepCount++;
                        current = new Coords(current.x, current.y - 1, stepCount);
                        visited.add(current);
                    }
                    break;
                case 'D':
                ...
                case 'L':
                    for (int i = 0; i < instruction.length; i++) {
                        stepCount++;
                        current = new Coords(current.x - 1, current.y, stepCount);
                        visited.add(current);
                    }
                    break;
                case 'R':
    ...
    public int findSmallestWeight(List<Coords> list1, List<Coords> list2) {
        int smallestWeight = Integer.MAX_VALUE;

        for (Coords coord1 : list1) {
            for (Coords coord2 : list2) {
                if (coord1.equals(coord2)) {
                    if (coord1.equals(ORIGIN) || coord2.equals(ORIGIN)) {
                        continue;
                    }
                    int weight = Distance.findWeight(coord1, coord2);
                    System.out.println(weight + " " + coord1 + " " + coord2);
