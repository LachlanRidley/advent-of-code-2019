...
public class Solver {
    ...
    public int solvinate(String puzzle) {
        ...
        List<Coords> collides = findCollisionPoints(line1Coords, line2Coords);

        int smallestDistance = Integer.MAX_VALUE;

        Coords origin = ORIGIN;

        for (Coords collide : collides) {
            if (collide.equals(ORIGIN)) {
                continue;
            }

            int distance = Distance.findManhattanDistance(origin, collide);
            ...
            switch (instruction.dir) {
                case 'U':
                    for (int i = 0; i < instruction.length; i++) {
                        current = new Coords(current.x, current.y - 1);
                        visited.add(current);
                    }
                    break;
                case 'D':
                ...
                case 'L':
                    for (int i = 0; i < instruction.length; i++) {
                        current = new Coords(current.x - 1, current.y);
                        visited.add(current);
                    }
                    break;
                case 'R':
    ...
    public List<Coords> findCollisionPoints(List<Coords> list1, List<Coords> list2) {
        List<Coords> collidy = new ArrayList<>();
